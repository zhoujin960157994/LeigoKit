...
struct AdapterFactoryParser {
    ...
    static func parseAdapterFactoryManager(_ config: Yaml) throws -> AdapterFactoryManager {
        ...
        for adapterConfig in adapterConfigs {
            ...
            case .some("vmess"):
                factoryDict[id] = try parseVmessAdapterFactory(adapterConfig)
            case nil:
    ...
    static func parseVmessAdapterFactory(_ config: Yaml) throws -> VmessAdapterFactory {
        guard let host = config["host"].string else {
            throw ConfigurationParserError.adapterParsingError(errorInfo: "Host (host) is required.")
        }
        ...
        guard let uuidStr = config["uuid"].string else {
            throw ConfigurationParserError.adapterParsingError(errorInfo: "UUID (uuid) is required.")
        }
        
        guard let uuid = UUID(uuidString: uuidStr) else {
            throw ConfigurationParserError.adapterParsingError(errorInfo: "UUID (uuid) format error.")
        }
        guard let path = config["path"].string else {
                   throw ConfigurationParserError.adapterParsingError(errorInfo: "path (path) is required.")
               }
        
        guard let alterId = config["alterId"].int else {
            throw ConfigurationParserError.adapterParsingError(errorInfo: "alterId (alterId) is required.")
        }
        
        return VmessAdapterFactory(serverHost: host, serverPort: port, uuid: uuid,alterId: alterId,path: path)
    }
