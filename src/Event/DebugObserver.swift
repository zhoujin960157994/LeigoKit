...
open class YanshiDebugTunnelObserver: Observer<TunnelEvent> {
    static var count: Int = 0
    override open func signal(_ event: TunnelEvent) {
        switch event {
        case .closed,
             .forceCloseCalled:
            YanshiDebugTunnelObserver.count-=1
            //NSLog("NEKit YanshiDebugTunnel signal close, event: \(event)")
        case .opened(let tunnel):
            YanshiDebugTunnelObserver.count+=1
            //NSLog("NEKit YanshiDebugTunnel signal open count: \(YanshiDebugTunnelObserver.count), event: \(event), Tunnel: \(tunnel)")
        default:
            _ = 0
        }
    }
...
open class YanshiDebugObserverFactory: DebugObserverFactory {
    public override init() {}

    override open func getObserverForTunnel(_ tunnel: Tunnel) -> Observer<TunnelEvent>? {
        return YanshiDebugTunnelObserver()
    }
}
