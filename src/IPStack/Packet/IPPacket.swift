...
open class IPPacket {
    ...
    init?(packetData: Data) {
        ...
        guard let vhl = scanner.readByte() else {
			NSLog("Got abnormal ip packet size")
			return
        }
        ...
        guard let tos = scanner.readByte() else {
          NSLog("Got unknown ip packet tos")
          return nil
        }

        guard let packetLength = scanner.read16() else {
          NSLog("Got unknown ip packet length")
          return nil
        }

        guard totalLength == packetLength else {
            NSLog("Packet length mismatches from header.")
            return nil
        }

        guard let identification = scanner.read16() else {
          NSLog("Got unknown ip packet identification")
          return nil
        }
        self.identification = identification

        guard let offset = scanner.read16() else {
          NSLog("Got unknown ip packet offset")
          return nil
        }
        self.offset = offset

        guard let TTL = scanner.readByte() else {
          NSLog("Got unknown ip packet TTL")
          return nil
        }
        self.TTL = TTL

        guard let byte = scanner.readByte() else {
          NSLog("Got unknown ip packet protocol")
          return nil
        }

        guard let proto = TransportProtocol(rawValue: byte) else {
        ...
        // ignore checksum
        guard let _ = scanner.read16() else {
          NSLog("Got unknown ip packet checksum")
          return nil
        }

        switch version {
        case .iPv4:
            guard let intSourceAddress = scanner.read32() else {
              NSLog("Got unknown ip packet ipv4 source addr")
              return nil
            }
            sourceAddress = IPAddress(ipv4InNetworkOrder: CFSwapInt32(intSourceAddress))
            guard let intDestinationAddress = scanner.read32() else {
              NSLog("Got unknown ip packet ipv4 dest addr")
              return nil
            }
            destinationAddress = IPAddress(ipv4InNetworkOrder: CFSwapInt32(intDestinationAddress))
        default:
