...
open class DNSServer: DNSResolverDelegate, IPStackProtocol {
    ...
    fileprivate func sendQueryToRemote(_ session: DNSSession) {
        ...
        queue.asyncAfter(deadline: DispatchTime.now() + DispatchTimeInterval.microseconds(2000)) {
            [weak self] in
            if Date().timeIntervalSince(self!.lastReceiveTime) > TimeInterval(2) {
                DDLogWarn("NEKit DNSServer resolve timeout, restarting... ")
