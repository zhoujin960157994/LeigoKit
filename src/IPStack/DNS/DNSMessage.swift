...
open class DNSMessage {
    ...
    init?(payload: Data) {
        ...
        if let status = DNSReturnStatus(rawValue: bytes & 0x0F) {
            ...
            DDLogError("NEKit Received DNS response with unknown status: \(bytes & 0x0F).")
        ...
        for _ in 0..<queryCount {
            let query = DNSQuery(payload: payload, offset: scanner.position, base: 0)
            if query == nil {
                DDLogError("NEKit Received DNS response with unknown DNSQuery.")
                return nil
            }
        ...
        for _ in 0..<answerCount {
            guard let resource = DNSResource(payload: payload, offset: scanner.position, base: 0) else {
                DDLogError("NEKit Received DNS response with unknown answer DNSResource.")
                return nil
            }
        ...
        for _ in 0..<nameserverCount {
            guard let resource = DNSResource(payload: payload, offset: scanner.position, base: 0) else {
                DDLogError("NEKit Received DNS response with unknown nameserver DNSResource.")
                return nil
            }
        ...
        for _ in 0..<addtionalCount {
            guard let resource = DNSResource(payload: payload, offset: scanner.position, base: 0) else {
                DDLogError("NEKit Received DNS response with unknown addtional DNSResource.")
                return nil
            }
...
open class DNSResource {
    ...
    init?(payload: Data, offset: Int, base: Int = 0) {
        ...
        if scanner.position+Int(dataLength) > payload.count {
            DDLogError("Received DNS packet too short!")
            NSLog("Receive DNS packet too short!")
            return nil
        }
