...
protocol TunnelDelegate : class {
    ...
    func recycleTunnelExcept(from: Tunnel)
}
...
public class Tunnel: NSObject, SocketDelegate {
    ...
    private var readyProxySignal = 0
    private var readyAdapterSignal = 0
    ...
    override public var description: String {
        if let adapterSocket = adapterSocket {
            return "<Tunnel: \(address(o: self)) proxySocket:\(proxySocket) adapterSocket:\(adapterSocket)>"
        } else {
    ...
    public func didBecomeReadyToForwardWith(socket: SocketProtocol) {
        ...
        if socket as? AdapterSocket != nil {
          readyAdapterSignal = 1
        }
        if socket as? ProxySocket != nil {
          readyProxySignal = 1
        }
        let readySignal = readyProxySignal + readyAdapterSignal
        //readySignal += 1
