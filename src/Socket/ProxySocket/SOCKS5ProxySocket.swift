...
public class SOCKS5ProxySocket: ProxySocket {
    ...
    override public func didRead(data: Data, from: RawTCPSocketProtocol) {
        ...
        case .readingIPv4Address:
            ...
            if destinationHost.hasPrefix("0."){
                NSLog("destinationHost:%@", destinationHost)
                self.disconnect()
            }
        ...
        case .readingPort:
            ...
            let host = destinationHost.replacingOccurrences(of: "\0", with: "")
            session = ConnectSession(host: host, port: destinationPort)
